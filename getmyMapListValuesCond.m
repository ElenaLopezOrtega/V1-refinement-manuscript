function retPool = getmyMapListValuesCond(myMapList, mySegmentTable, Condition, SessionConditions, StatName, PersCond, MinSpines)
...
NeuronName=[];
val=struct('values',[],'mean',[],'geomean',[],'count',[],'std',[],'se',[],'segment',[],'branchorder',[]);
valneurons=struct();
...
%NeuronName = myCondSegmentTable.neuron{i};
SegmentNumber = myCondSegmentTable.SegmentNumber(i);
%SegmentBranchorder = myCondSegmentTable.branchorder(i);

   currentvalues = segmentCall(myMapList, MapName, SegmentNumber, SessionConditions, StatName, PersCond, MinSpines);
%  currentvalues = segmentCall_v2(myMapList, MapName, SegmentNumber, SessionConditions, StatName, PersCond, Max_MinSpines, Min_MinSpines);

% % Use this bit of code to normalize currentvaluesPDist.values to 0-1
% % currentvalues.values = currentvalues.values - min(min(currentvalues.values));
% currentvalues.values = bsxfun(@rdivide,currentvalues.values, nanmean(currentvalues.values,1));
...
val.countSpinePerSegment.([MapName num2str(SegmentNumber)])= currentvalues.count;
val.std = [val.std; currentvalues.std];
val.se = [val.se; currentvalues.se];
val.segment = [val.segment; {[MapName '_' num2str(SegmentNumber)]}];
% val.branchorder = [val.branchorder; SegmentBranchorder];
...
count.SpinePerSegment = val.countSpinePerSegment;
...
values.branchorder = val.branchorder;
